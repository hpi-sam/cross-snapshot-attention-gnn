...
def generate_random_3graph(size_H, K=3):
    ...
    for i, node1 in enumerate(first):
        ...
        H[node1_1][
            helpers.get_rank(node1_1, H)
        ] = node1  # Create the edge (node1_1, node1)

        H[node1][1] = node1_2  # Create the edge (node1, node1_2)
        H[node1_2][
            helpers.get_rank(node1_2, H)
        ] = node1  # Create the edge (node1_2, node1)

        H[node1][2] = node1_3  # Create the edge (node1, node1_3)
        H[node1_3][
            helpers.get_rank(node1_3, H)
        ] = node1  # Create the edge (node1_3, node1)
