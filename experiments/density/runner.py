...
def analyze_results(results: pd.DataFrame, path: str):
    ...
    compute_means_with_significance(
        results=results, group_col="edge_prob", path=path)
    for dataset_name, dataset_df in results.groupby("dataset"):
        ...
        compute_means_with_significance(
            results=dataset_df, group_col="edge_prob", path=dataset_path)
        ...
        for metric, metric_df in dataset_df.groupby("metric"):
            ...
            for j, (graph, graph_type_df) in enumerate(metric_df.groupby("graph")):
                ...
                sns.lineplot(data=graph_type_df, x="edge_prob",
                             y="value", hue="model", ax=ax[j], errorbar=None, legend=False, linewidth=2.5)
                ax[j].set_axisbelow(True)
                ax[j].set_title(f"{graph}")
                ax[j].set_xlabel("density")
                ax[j].set_ylabel(f"{metric} (%)" if metric !=
                                 "runtime" else "runtime (s)")
                ax[j].grid(True)
            save_fig(
                fig, f"{dataset_path}/linechart_{metric}", svg=True)

            # add another combined chart

            metric_df["value"] = metric_df["value"] * \
                100 if metric != "runtime" else metric_df["value"] / 1000
            ax = sns.lineplot(data=metric_df, x="edge_prob",
                              y="value", hue="model", errorbar=None, legend=metric == "accuracy" and dataset_name in ["COV-19", "FAKE"], linewidth=2.5)
            ...
            if dataset_name == "FAKE":
                ...
                plt.legend(loc='lower center', bbox_to_anchor=(0.35, 0))
            else:
                legend = ax.legend_
...
def create_dataset(dataset_class, graph_type: str, edge_prob: float, recreate=False):
    dataset = dataset_class(
        ...
        behavior_edge_probability=edge_prob,
        recreate=recreate,
    )
    return dataset
...
def run_experiment(exp_args):
    ...
    dataset = create_dataset(
        dataset_class=dataset_class, graph_type=graph_type, edge_prob=edge_prob, recreate=recreate
    )
...
def run_experiments(recreate=False, analyze=True, execute=True, parallelize=False, **kwargs):
    path = "experiments/density"
    results = read_results(
        path, [*default_result_columns(), "graph", "edge_prob"])
    if execute:
        models = get_models()
        edge_probs = [0.1, 0.3, 0.5, 0.7, 0.9]
